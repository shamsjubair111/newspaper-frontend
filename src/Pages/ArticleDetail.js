...
const ArticleDetail = () => {
  ...
  const [editForm, setEditForm] = useState({ title: '', articleAuthor: '', content: '' });
  ...
  useEffect(() => {
    ...
    const doTranslate = async () => {
      setTranslating(true);
      try {
        const authorName = article.articleAuthor?.name || '';
        const [title, translatedAuthor, content] = await Promise.all([
          translateText(article.title),
          authorName ? translateText(authorName) : Promise.resolve(''),
        ...
        setTranslatedArticle({
          ...
          articleAuthor: article.articleAuthor
            ? { ...article.articleAuthor, name: translatedAuthor }
            : null,
        });
      } catch {
        setTranslatedArticle(null);
  ...
  const openEditModal = async () => {
    setEditForm({
      title: article.title || '',
      articleAuthor: article.articleAuthor?._id || '',
    ...
    try {
      ...
      const res = await fetch(`${process.env.REACT_APP_API_URL}/api/articles/${id}`, {
        ...
        body: JSON.stringify({
          ...
          articleAuthor: editForm.articleAuthor,
      ...
      const selectedAuthor = authors.find(a => a._id === editForm.articleAuthor);
      setArticle(prev => ({
        ...prev,
        title: editForm.title,
        articleAuthor: selectedAuthor || prev.articleAuthor,
        content: editForm.content,
      }));
      setTranslatedArticle(null);
  ...
  const articleAuthor = displayArticle?.articleAuthor;
                ...
                <div className="cmt-form-inner">
                  ...
                  <form className="cmt-form" onSubmit={handleCommentSubmit}>
                    ...
                    <div className="cmt-form-footer">
                      ...
                      <button type="submit" className="cmt-submit-btn" disabled={commentLoading || !commentText.trim()}>
                        {commentLoading ? (
          ...
          <div className="ad-edit-modal">
            ...
            <form onSubmit={handleEditSubmit} className="ad-edit-form">
              ...
              <div className="ad-edit-field">
                ...
                <input
                  className="ad-edit-input" type="text"
                  value={editForm.title}
                  onChange={(e) => setEditForm(f => ({ ...f, title: e.target.value }))}
                  placeholder="Article title..." required disabled={editLoading}
                />
              </div>

              <div className="ad-edit-field">
                <label className="ad-edit-label">Author</label>
                <select
                  className="ad-edit-input"
                  value={editForm.articleAuthor}
                  onChange={(e) => setEditForm(f => ({ ...f, articleAuthor: e.target.value }))}
                  disabled={editLoading || authors.length === 0}
                >
              ...
              <div className="ad-edit-field">
                ...
                <textarea
                  className="ad-edit-textarea" rows="12"
